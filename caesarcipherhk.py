#Tests on Caesar Cipher Hk from big book of small python projects
#If remove the translated = '' line, we get an error translated is not defined
#We get the same encrypted caesar cipher message on every key if we have both translated = translated + symbol
#If we enter an unecrypted message it copy this same message on every key

# -> see the other book Cracking Code with python to check how to detect english message
# -> check how to see the runtime for python code + how to optimize it
# -> look for other cipher and hk
